          ...
          <div class="col-lg-6">
            <div class="table-responsive">
              <table class="table table-borderless mb-0">
                <thead class="border-bottom border-dark border-top-0 border-start-0 border-end-0 border-2 text-nowrap">
                  ...
                  <template v-for="item in cart.carts" :key="item.id">
                    <tr class="align-middle border-bottom border-dark">
                      <th scope="row" class="fw-normal bg-wave">
                        <img :src="item.product.imageUrl" :alt="item.product.title" style="width: 100px;height: 100px;">
                      </th>
                      <td class="bg-wave">{{ item.product.title }}</td>
                      <td class="bg-wave">{{ item.qty }}</td>
                      <td class="bg-wave">NT$ <span class="text-notoSans">{{ item.product.price }}</span></td>
                    </tr>
                  </template>
                </tbody>
              </table>
            </div>
            <p class="text-end fw-bold fs-5 mt-16 mb-0"><span class="me-8">總計：</span>NT$ <span class="text-notoSans">{{ Math.round(cart.final_total) }}</span></p>
            ...
            <div class="border border-gray bg-white rounded-2 p-20">
              ...
              <v-form ref="form" v-slot="{ errors }" @submit="submitOrder">
                ...
                <div class="mb-16">
                  ...
                  <v-field id="tel" name="手機" type="tel" class="form-control border-gray" v-model="form.user.tel"
                    :class="{ 'is-invalid': errors['手機'] }" placeholder="請輸入手機號碼" :rules="isPhone"></v-field>
                ...
                <div class="mb-16">
                  ...
                  <v-field id="email" name="email" type="email" class="form-control border-gray"
                    v-model="form.user.email" :class="{ 'is-invalid': errors['email'] }" placeholder="請輸入 Email"
                ...
                <div class="mb-16">
                  ...
                  <v-field id="address" name="地址" type="text" class="form-control border-gray"
                    v-model="form.user.address" :class="{ 'is-invalid': errors['地址'] }" placeholder="請輸入地址"
                    rules="required" ref="address"></v-field>
                  <error-message name="地址" class="invalid-feedback fw-bold"></error-message>
                </div>
                <div class="mb-24">
                  <label for="備註" class="form-label">備註</label>
                  <v-field as="textarea" id="備註" name="備註" class="form-control border-gray" v-model="form.message"
                    :class="{ 'is-invalid': errors['備註'] }" style="height: 150px;"></v-field>
                </div>
